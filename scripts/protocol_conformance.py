...
def main():
    ...
    # 工具集按分支不同：
    #   fast=8（合并文件类+无 scan/batch/dir）
    #   lite=11（ssh_filesystem 合并 + 无 scan，保留 batch/dir 传输）
    #   main=15（完整）
    EXPECTED_TOOLS = sys.argv[2] if len(sys.argv) > 2 else "fast"
    if EXPECTED_TOOLS == "main":
        expected = {
    ...
    elif EXPECTED_TOOLS == "lite":
        expected = {
            "ssh_exec", "ssh_exec_batch", "ssh_list_hosts",
            "ssh_upload", "ssh_download", "ssh_upload_dir", "ssh_download_dir",
            "ssh_filesystem",
            "ssh_get_review_mode", "ssh_set_review_mode", "ssh_get_audit_logs",
        }
